...
input = '''[1518-02-15 23:58] Guard #3557 begins shift
...
def parse_input(input):
    ...
    for l in inp:
        if l[19:24] == 'Guard':
            ...
            d[guard_id].append((beg, end))
        is_awake = not is_awake
    return d


def gethistmax(id_span_list):
    hist = [0 for _ in range(60)]
    for beg, end in id_span_list[1]:
        for i in range(beg, end):
            hist[i] += 1
    return max(hist), hist


def part0(input, maxfunction):
    rec = max(input.items(), key=maxfunction)
    guard_id = rec[0]
    maxasleep, hist = gethistmax(rec)
    sleepy_minute = hist.index(maxasleep)
    return guard_id*sleepy_minute


def part1(input):
    def maxfunction(id_span_list):
        return sum((end - beg) for beg, end in id_span_list[1])

    return part0(input, maxfunction)


def part2(input):
    def maxfunction(id_span_list):
        return gethistmax(id_span_list)[0]
...
if __name__ == '__main__':
    ...
    print(138280 == part1(inp))

    print(89347 == part2(inp))
