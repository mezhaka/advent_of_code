...
input ='''position=< 50769, -40375> velocity=<-5,  4>
...
def parse_input(input):
    ret = []
    intre = r'[-+]?[0-9]+'
    pointre = r'<\s*({0}),\s+({0})>'.format(intre)
    regex = r'{0}.+{0}'.format(pointre)
    for l in input.split('\n'):
        m = re.search(regex, l)
        ret.append(([int(m.group(1)), int(m.group(2))],
                    (int(m.group(3)), int(m.group(4)))))
    return ret  # [([x, y], (vel_x, vel_y))]


def get_second_pos(pos_vel):
    return pos_vel[0][1]


def move(delta_time, coords):
    min_x = min_y = sys.maxsize
    max_x = max_y = -min_x
    for pos, vel in coords:
    ...
    coords.sort(key=get_second_pos)
    return ((min_x, min_y), (max_x, max_y))


def iscontingent(coords):
    return all(abs(left[0][1] - right[0][1]) <= 1
...
def sky(coords, box):
    ...
    lenx = max_x - min_x + 1
    leny = max_y - min_y + 1
    sky = [[' ' for i in range(leny)] for j in range(lenx)]
    for (x, y), vel in coords:
        normx = x - min_x
        normy = y - min_y
        sky[normx][normy] = 'X'
    return '\n'.join(''.join(sky[i]) for i in range(len(sky)))
...
if __name__ == '__main__':
    ...
    for i in range(1, 100000):
        box = move(1, coords)
        if iscontingent(coords):
            print('========================================{}'.format(i))
            print(sky(coords, box))
